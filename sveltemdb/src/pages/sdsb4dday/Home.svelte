<script>
    ...
    const schema = yup.object().shape({
        date_keluaran: yup.string().required("The Date Keluaran is required")
    });
    const { form, errors, handleChange, handleSubmit } = createForm({
        ...
        onSubmit:(values) => {
            handleSave(values.date_keluaran)
        }
    })
    const NewData = () => {
        clearField()
    ...
    async function handleSave(date_keluaran) {
    ...
    async function handleSaveGenerator(tipe,prize) {
        ...
        if (json.status == 200) {
            ...
            switch(tipe){
                case "prize1":
                    prize1_save_flag = true;
                    prize1_flag = true;break;
                case "prize2":
                    prize2_save_flag = true;
                    prize2_flag = true;break;
                case "prize3":
                    prize3_save_flag = true;
                    prize3_flag = true;break;
            }
        } else if(json.status == 403){
            alert(json.message)
        } else {
    ...
    $:{
        if ($errors.date_keluaran){
            alert($errors.date_keluaran+"\n")
            $form.date_keluaran = ""
        }
    }
	...
	<slot:template slot="body">
        <div class="mb-3">
			...
			<Input
                ...
                bind:value={$form.date_keluaran}
                invalid={$errors.date_keluaran.length > 0}
                type="date"
                name="date"
                id="exampleDate"
