<script>
	...
	let title_page = "JAPAN - NIGHT 17:00"
    let sData = "";
    ...
    async function handleSave() {
        ...
        if(flag){
            const res = await fetch("/api/savejapan", {
                method: "POST",
                headers: {
    ...
    async function handleSaveGenerator(tipe,prize) {
        ...
        if(flag_save){
            const res = await fetch("/api/savegeneratorjapan", {
                method: "POST",
                headers: {
                    "Content-Type": "application/json",
                    Authorization: "Bearer " + token,
    ...
    async function handleGeneratorAutomation() {
        const res = await fetch("/api/generatornumberjapan", {
            method: "POST",
            headers: {
                "Content-Type": "application/json",
                Authorization: "Bearer " + token,
                                ...
                                {#each listHome as rec }
                                    <tr>
                                        <td NOWRAP style="text-align: center;vertical-align: top;cursor:pointer;">
                                            <i on:click={() => {
                                                    EditData(rec.japan_id,rec.japan_date,
                                                    rec.japan_prize1,
                                                    rec.japan_prize2,
                                                    rec.japan_prize3);
                                                }} class="bi bi-pencil"></i>
                                        </td>
                                        <td NOWRAP style="text-align: center;vertical-align: top;font-size: {table_body_font};">{rec.japan_no}</td>
                                        <td NOWRAP style="text-align: center;vertical-align: top;font-size: {table_body_font};">{rec.japan_date}</td>
                                        <td NOWRAP style="text-align: left;vertical-align: top;font-size: {table_body_font};">#{rec.japan_id}</td>
                                        <td NOWRAP style="text-align: center;vertical-align: top;font-size: {table_body_font};">{rec.japan_prize1}</td>
                                        <td NOWRAP style="text-align: center;vertical-align: top;font-size: {table_body_font};">{rec.japan_prize2}</td>
                                        <td NOWRAP style="text-align: center;vertical-align: top;font-size: {table_body_font};">{rec.japan_prize3}</td>
                                        <td NOWRAP style="text-align: left;vertical-align: top;font-size: {table_body_font};">{rec.japan_create}</td>
                                        <td NOWRAP style="text-align: left;vertical-align: top;font-size: {table_body_font};">{rec.japan_update}</td>
                                    </tr>
                                {/each}
                            </tbody>
                            {:else}
        ...
        <div class="mb-3">
            <label for="exampleForm" class="form-label">
                <div class="d-flex flex-row">
                    ...
                    <div class="ps-2 mt-3" style="font-size: 11px;color:blue;">OPEN: 17:55 - 18:00</div>
                </div>
            </label>
